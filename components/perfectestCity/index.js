// components/perfectestCity/index.js
...
Component({
  ...
  lifetimes: {
    attached() {
      ...
      this.setData({ adcode: this.data.addressInfo.cityId, location: this.data.location })
  ...
  methods: {
    ...
    back() {
      // wx.navigateBack()
      this.triggerEvent('closeCityShow')
    },
    // 选择省市区街道
    SELECTED(event) {
    ...
    comfirmScreening() {
      ...
      if (this.data.type == 1) {
        ...
        this.triggerEvent('changeCity',selectedClty)
      } else {
        if (this.data.selectedList[this.data.selectedList.length - 1].type == 'province') return showToast('小优提醒您,至少选择到市哦')
